...
public class JabRefFrame extends JFrame implements OutputPrinter {
	...
	AbstractAction close = new CloseDatabaseAction(),
			...
			editEntry = new GeneralAction("edit", "Edit entry",
					Globals.lang("Edit entry"), prefs.getKey("Edit entry")),
			...
			redo = new GeneralAction("redo", "Redo", Globals.lang("Redo"),
					prefs.getKey("Redo")),
			forward = new GeneralAction("forward", "Forward",
					Globals.lang("Forward"), "right", prefs.getKey("Forward")),
			 ...
			 */
			delete = new GeneralAction("delete", "Delete",
					Globals.lang("Delete"), prefs.getKey("Delete")),
			/*
			...
			mark = new GeneralAction("markEntries", "Mark entries",
					Globals.lang("Mark entries"), prefs.getKey("Mark entries")),
			...
			toggleRelevance = new GeneralAction(Relevance.getInstance()
					.getValues().get(0).getActionName(), Relevance
					.getInstance().getValues().get(0).getMenuString(),
					Relevance.getInstance().getValues().get(0).getToolTipText()),
			toggleQualityAssured = new GeneralAction(Quality.getInstance()
					.getValues().get(0).getActionName(), Quality.getInstance()
					.getValues().get(0).getMenuString(), Quality.getInstance()
					.getValues().get(0).getToolTipText()),
			// priority = new GeneralAction("setPriority", "Set priority",
			// Globals.lang("Set priority")),
			manageSelectors = new GeneralAction("manageSelectors",
					"Manage content selectors"),
			saveSessionAction = new SaveSessionAction(),
			loadSessionAction = new LoadSessionAction(),
			incrementalSearch = new GeneralAction("incSearch",
					"Incremental search",
					Globals.lang("Start incremental search"),
					prefs.getKey("Incremental search")),
			normalSearch = new GeneralAction("search", "Search",
					Globals.lang("Search"), prefs.getKey("Search")),
			toggleSearch = new GeneralAction("toggleSearch", "Search",
					Globals.lang("Toggle search panel")),
			...
			copyCiteKey = new GeneralAction("copyCiteKey",
			...
			editStrings = new GeneralAction("editStrings", "Edit strings",
					Globals.lang("Edit strings"), prefs.getKey("Edit strings")),
			...
			toggleHighlightAny = new GeneralAction(
					"toggleHighlightGroupsMatchingAny",
			...
			toggleHighlightAll = new GeneralAction(
					"toggleHighlightGroupsMatchingAll",
			...
			makeKeyAction = new GeneralAction("makeKey",
			...
			openFile = new GeneralAction("openExternalFile", "Open file",
					Globals.lang("Open file"), prefs.getKey("Open file")),
			...
			openUrlAboutJabref = new GeneralAction("openUrlAboutJabref", "JabRef Website",
					Globals.lang("JabRef Website"),
					prefs.getKey("JabRef Website")),
			openSpires = new GeneralAction("openSpires", "Open SPIRES entry",
					Globals.lang("Open SPIRES entry"),
					prefs.getKey("Open SPIRES entry")),
			/*
			 * It looks like this wasn't being implemented for spires anyway so
			 * we comment it out for now.
			 * 
			 * openInspire = new GeneralAction("openInspire",
			 * "Open INSPIRE entry", Globals.lang("Open INSPIRE entry"),
			...
			exportToClipboard = new GeneralAction("exportToClipboard",
					"Export selected entries to clipboard"),
			// expandEndnoteZip = new ExpandEndnoteFilters(this),
			autoSetPdf = new GeneralAction("autoSetPdf", Globals.lang(
					"Synchronize %0 links", "PDF"),
					Globals.prefs.getKey("Synchronize PDF")),
			autoSetPs = new GeneralAction("autoSetPs", Globals.lang(
					"Synchronize %0 links", "PS"),
					Globals.prefs.getKey("Synchronize PS")),
			autoSetFile = new GeneralAction("autoSetFile",
					Globals.lang("Synchronize file links"),
					Globals.prefs.getKey("Synchronize files")),
			...
			resolveDuplicateKeys = new GeneralAction("resolveDuplicateKeys",
	...
	NewEntryAction newEntryAction = new NewEntryAction(
			prefs.getKey("New entry"));
	NewEntryAction[] newSpecificEntryAction = new NewEntryAction[] {
			...
			new NewEntryAction("mastersthesis",
					prefs.getKey("New mastersthesis")),
			new NewEntryAction("proceedings", prefs.getKey("New proceedings")),
			new NewEntryAction("inproceedings"),
			new NewEntryAction("conference"),
			new NewEntryAction("incollection"), new NewEntryAction("booklet"),
			new NewEntryAction("manual"), new NewEntryAction("techreport"),
			new NewEntryAction("unpublished", prefs.getKey("New unpublished")),
			new NewEntryAction("misc"), new NewEntryAction("other") };
	...
	private void init() {
		tabbedPane = new DragDropPopupPane(manageSelectors, databaseProperties,
				bibtexKeyPattern);

		macOSXRegistration();

		UIManager.put("FileChooser.readOnly",
				Globals.prefs.getBoolean("filechooserDisableRename"));
		...
		if (!prefs.getBoolean("windowMaximised")) {
			 ...
			 * 
			 * 
			 * // Get size of each screen for (int i=0; i<gs.length; i++) {
			 * DisplayMode dm = gs[i].getDisplayMode(); int screenWidth =
			 * dm.getWidth(); int screenHeight = dm.getHeight();
			...
			if (GraphicsEnvironment.getLocalGraphicsEnvironment()
				...
				// Make sure we are not above or to the left of the screen
				// bounds:
				if (posX < bounds.x)
					posX = bounds.x;
				if (posY < bounds.y)
		...
		tabbedPane.addChangeListener(new ChangeListener() {
			public void stateChanged(ChangeEvent e) {
				...
				if (bp != null) {
					groupToggle.setSelected(sidePaneManager
							.isComponentVisible("groups"));
					searchToggle.setSelected(sidePaneManager
							.isComponentVisible("search"));
					previewToggle.setSelected(Globals.prefs
							.getBoolean("previewEnabled"));
					highlightAny.setSelected(Globals.prefs
							.getBoolean("highlightGroupsMatchingAny"));
					highlightAll.setSelected(Globals.prefs
							.getBoolean("highlightGroupsMatchingAll"));
					Globals.focusListener.setFocused(bp.mainTable);
					setWindowTitle();
					// Update search autocompleter with information for the
					// correct database:
		...
		// The following sets up integration with Unity's global menu, but
		// currently (Nov 18, 2012)
	...
	public void setWindowTitle() {
		...
		if (bp.getFile() != null) {
			setTitle(GUIGlobals.frameTitle + " - " + bp.getFile().getPath()
					+ star);
		} else {
			setTitle(GUIGlobals.frameTitle + " - " + Globals.lang("untitled")
					+ star);
		}
	}
	...
	private void initSidePane() {
		 ...
		 */
		JabRefPlugin jabrefPlugin = JabRefPlugin.getInstance(PluginCore
				.getManager());
		if (jabrefPlugin != null) {
			for (EntryFetcherExtension ext : jabrefPlugin
				...
				try {
					...
					PluginCore.getManager().disablePlugin(
							ext.getDeclaringPlugin().getDescriptor());
					ex.printStackTrace();
				}
			}
		}
	...
	public void about() {
		JDialog about = new JDialog(JabRefFrame.this,
				Globals.lang("About JabRef"), true);
		...
		try {
			...
			JOptionPane.showMessageDialog(JabRefFrame.this,
					"Could not load file 'About.html'", "Error",
	...
	public void quit() {
			...
			for (int i = 0; i < tabbedPane.getTabCount(); i++) {
				if (baseAt(i).isBaseChanged()) {
					...
					int answer = JOptionPane.showConfirmDialog(
							JabRefFrame.this, Globals
									.lang("Database has changed. Do you "
		...
		if (close) {
			...
			prefs.putBoolean("searchPanelVisible",
					sidePaneManager.isComponentVisible("search"));
	...
	private void initLayout() {
		tabbedPane.putClientProperty(Options.NO_CONTENT_BORDER_KEY,
				Boolean.TRUE);
		 ...
		 * gbl.setConstraints(empt, con); getContentPane().add(empt);
		 * 
	...
	private void markActiveBasePanel() {
		...
		if ((lastTabbedPanelSelectionIndex > -1)
				&& (lastTabbedPanelSelectionIndex < len))
			tabbedPane.setForegroundAt(lastTabbedPanelSelectionIndex,
					GUIGlobals.inActiveTabbed);
		if ((now > -1) && (now < len))
	...
	class GeneralAction extends MnemonicAwareAction {
		...
		public GeneralAction(String command, String text, String description,
				String imageName, KeyStroke key) {
	...
	private void fillMenu() {
		...
		edit.addSeparator();
		if (Globals.prefs
				.getBoolean(SpecialFieldsUtils.PREF_SPECIALFIELDSENABLED)) {
			JMenu m;
			if (Globals.prefs
					.getBoolean(SpecialFieldsUtils.PREF_SHOWCOLUMN_RANKING)) {
				m = new JMenu();
				RightClickMenu.populateSpecialFieldMenu(m, Rank.getInstance(),
						this);
				edit.add(m);
			}
			if (Globals.prefs
					.getBoolean(SpecialFieldsUtils.PREF_SHOWCOLUMN_RELEVANCE)) {
				edit.add(toggleRelevance);
			}
			if (Globals.prefs
					.getBoolean(SpecialFieldsUtils.PREF_SHOWCOLUMN_QUALITY)) {
				edit.add(toggleQualityAssured);
			}
			if (Globals.prefs
					.getBoolean(SpecialFieldsUtils.PREF_SHOWCOLUMN_PRIORITY)) {
				m = new JMenu();
				RightClickMenu.populateSpecialFieldMenu(m,
						Priority.getInstance(), this);
				edit.add(m);
			}
		}
		edit.addSeparator();
		...
		search.addSeparator();
		GeneralFetcher generalFetcher = new GeneralFetcher(sidePaneManager,
				this, fetchers);
		 ...
		 * actionPerformed(ActionEvent e) { Font f=new FontSelectorDialog
		 * (JabRefFrame.this, GUIGlobals.CURRENTFONT).getSelectedFont();
		 * if(f==null) return; else GUIGlobals.CURRENTFONT=f; // updatefont
		 * prefs.put("fontFamily", GUIGlobals.CURRENTFONT.getFamily());
		 * prefs.putInt("fontStyle", GUIGlobals.CURRENTFONT.getStyle());
		 * prefs.putInt("fontSize", GUIGlobals.CURRENTFONT.getSize()); if
		 * (tabbedPane.getTabCount() > 0) { for (int i=0;
		 * i<tabbedPane.getTabCount(); i++) { baseAt(i).entryTable.updateFont();
		...
		helpMenu.addSeparator();
		helpMenu.add(about);
		helpMenu.add(openUrlAboutJabref);
		mb.add(helpMenu);
	}
	...
	public void addParserResult(ParserResult pr, boolean raisePanel) {
		if (pr.toOpenTab()) {
			...
			if (panel == null) {
				...
				addTab(pr.getDatabase(), pr.getFile(), pr.getMetaData(),
						pr.getEncoding(), raisePanel);
			} else {
				List<BibtexEntry> entries = new ArrayList<BibtexEntry>(pr
	...
	protected void initActions() {
		...
		openDatabaseOnlyActions.addAll(Arrays.asList(new Object[] {
				manageSelectors, mergeDatabaseAction, newSubDatabaseAction,
				close, save, saveAs, saveSelectedAs, undo, redo, cut, delete,
				copy, paste, mark, unmark, unmarkAll, editEntry, selectAll,
				copyKey, copyCiteKey, copyKeyAndTitle, editPreamble,
				editStrings, toggleGroups, toggleSearch, makeKeyAction,
				normalSearch, incrementalSearch, replaceAll, importMenu,
				exportMenu,
				/*
				 * openSpires wasn't being supported so no point in supporting
				 * openInspire
				 */
				openPdf, openUrl, openFile, openSpires, /*
																		 * openInspire
																		 * ,
																		 */
				togglePreview, dupliCheck, /* strictDupliCheck, */
				highlightAll, highlightAny, newEntryAction, plainTextImport,
				massSetField, manageKeywords, closeDatabaseAction,
				switchPreview, integrityCheckAction, autoSetPdf, autoSetPs,
				toggleHighlightAny, toggleHighlightAll, databaseProperties,
				abbreviateIso, abbreviateMedline, unabbreviate, exportAll,
				exportSelected, importCurrent, saveAll, dbConnect, dbExport,
				focusTable }));
		...
		severalDatabasesOnlyActions.addAll(Arrays.asList(new Action[] {
				nextTab, prevTab, sortTabs }));
	...
	public BasePanel addTab(BibtexDatabase db, File file, MetaData metaData,
			String encoding, boolean raisePanel) {
		...
		BasePanel bp = new BasePanel(JabRefFrame.this, db, file, metaData,
				encoding);
	...
	public void addTab(BasePanel bp, File file, boolean raisePanel) {
		...
		if (file == null) {
			...
			if (!bp.database().getEntries().isEmpty()) {
				...
				// the database came from an import and has to be treated
				// somehow
				// -> mark as changed
				// This also happens internally at basepanel to ensure
				// consistency
				title = title + "*";
			}
		} else {
				...
				JOptionPane
						.showMessageDialog(
								...
								Globals.lang("Your new key bindings have been stored.")
										+ "\n"
										+ Globals
												.lang("You must restart JabRef for the new key "
		...
		public void actionPerformed(ActionEvent e) {
			...
			if (basePanel().isBaseChanged()) {
				int answer = JOptionPane
						.showConfirmDialog(
								JabRefFrame.this,
								Globals.lang("Database has changed. Do you want to save "
										+ "before closing?"),
								Globals.lang("Save before closing"),
								JOptionPane.YES_NO_CANCEL_OPTION);
				if ((answer == JOptionPane.CANCEL_OPTION)
	...
	class NewDatabaseAction extends MnemonicAwareAction {
		...
		public void actionPerformed(ActionEvent e) {
			...
			addTab(database, null, new MetaData(),
	...
	class NewSubDatabaseAction extends MnemonicAwareAction {
		...
		public void actionPerformed(ActionEvent e) {
			...
			FromAuxDialog dialog = new FromAuxDialog(JabRefFrame.this, "",
					true, JabRefFrame.this.tabbedPane);
			...
			if (dialog.okPressed()) {
				BasePanel bp = new BasePanel(JabRefFrame.this,
						...
						null, // file
						new MetaData(), Globals.prefs.get("defaultEncoding")); // meta
																				// data
	...
	class IntegrityCheckAction extends AbstractAction {
		public IntegrityCheckAction() {
			super(Globals.menuTitle("Integrity check"), GUIGlobals
					.getImage("integrityCheck"));
		...
		public void actionPerformed(ActionEvent e) {
			...
			if (selComp != null) {
				...
				if (refBase != null) {
					IntegrityWizard wizard = new IntegrityWizard(
							JabRefFrame.this, basePanel());
	 ...
	 */
	public void addImportedEntries(final BasePanel panel,
			final List<BibtexEntry> entries, String filename,
		...
		if (Globals.prefs.getBoolean("useImportInspectionDialog")
				&& (Globals.prefs
						.getBoolean("useImportInspectionDialogForSingle") || (entries
						.size() > 1))) {
			SwingUtilities.invokeLater(new Runnable() {

				public void run() {
					ImportInspectionDialog diag = new ImportInspectionDialog(
							JabRefFrame.this, panel,
							BibtexFields.DEFAULT_INSPECTION_FIELDS, Globals
									.lang("Import"), openInNew);
					diag.addEntries(entries);
					diag.entryListComplete();
					// On the one hand, the following statement could help at
					// issues when JabRef is minimized to the systray
					// On the other hand, users might dislake modality and this
					// is not required to let the GUI work.
	...
	public int addBibEntries(List<BibtexEntry> bibentries, String filename,
		...
		if (bibentries == null || bibentries.size() == 0) {
			...
			JOptionPane.showMessageDialog(JabRefFrame.this, Globals
					.lang("No entries found. Please make sure you are "
							+ "using the correct import filter."),
		...
		Util.setAutomaticFields(bibentries,
				...
				Globals.prefs.getBoolean("markImportedEntries"));
		...
		if (intoNew || (tabbedPane.getTabCount() == 0)) {
			...
			for (BibtexEntry entry : bibentries) {
				try {
					...
					// ignore
					System.err
							.println("KeyCollisionException [ addBibEntries(...) ]");
				}
			}
			// Metadata are only put in bibtex files, so we will not find it
			// in imported files. We therefore pass in an empty MetaData:
			BasePanel bp = new BasePanel(JabRefFrame.this, database, null,
					new MetaData(), Globals.prefs.get("defaultEncoding"));
			/*
			...
			mainLoop: for (BibtexEntry entry : bibentries) {
				...
				if (checkForDuplicates) {
					loop: for (Iterator<String> i2 = database.getKeySet()
							.iterator(); i2.hasNext();) {
						BibtexEntry existingEntry = database.getEntryById(i2
								.next());
						if (DuplicateCheck.isDuplicate(entry, existingEntry)) {
							DuplicateResolverDialog drd = new DuplicateResolverDialog(
									JabRefFrame.this, existingEntry, entry,
							...
							if (res == DuplicateResolverDialog.KEEP_LOWER) {
								...
								ce.addEdit(new UndoableRemoveEntry(database,
										existingEntry, basePanel));
	...
	private void setUpImportMenu(JMenu importMenu, boolean intoNew_) {
		...
		SortedSet<ImportFormat> customImporters = Globals.importFormatReader
		...
		// Put in all formatters registered in ImportFormatReader:
		for (ImportFormat imFo : Globals.importFormatReader
			...
			if (tabbedPane.getTabCount() > 0) {
				for (int i = 0; i < tabbedPane.getTabCount(); i++) {
					if (tabbedPane.getTitleAt(i).equals(
							GUIGlobals.untitledTitle)) {
						tabbedPane.setSelectedIndex(i);
						int answer = JOptionPane
								.showConfirmDialog(
										JabRefFrame.this,
										Globals.lang("This untitled database must be saved first to be "
	...
	class ChangeTabAction extends MnemonicAwareAction {
		...
		public ChangeTabAction(boolean next) {
			...
			putValue(
					ACCELERATOR_KEY,
					(next ? prefs.getKey("Next tab") : prefs
							.getKey("Previous tab")));
		}
	...
	class EditAction extends MnemonicAwareAction {
		...
		public void actionPerformed(ActionEvent e) {
			...
			try {
				source.getActionMap().get(command)
						.actionPerformed(new ActionEvent(source, 0, command));
	...
	class CustomizeExportsAction extends MnemonicAwareAction {
		...
		public void actionPerformed(ActionEvent e) {
			ExportCustomizationDialog ecd = new ExportCustomizationDialog(
					JabRefFrame.this);
			ecd.setVisible(true);
		}
	}
	...
	class CustomizeImportsAction extends MnemonicAwareAction {
		...
		public void actionPerformed(ActionEvent e) {
			ImportCustomizationDialog ecd = new ImportCustomizationDialog(
					JabRefFrame.this);
			ecd.setVisible(true);
		}
	}
	...
	class DatabasePropertiesAction extends MnemonicAwareAction {
		...
		public void actionPerformed(ActionEvent e) {
			if (propertiesDialog == null)
				propertiesDialog = new DatabasePropertiesDialog(
						JabRefFrame.this);
	...
	class BibtexKeyPatternAction extends MnemonicAwareAction {
		...
		public void actionPerformed(ActionEvent e) {
			...
			if (bibtexKeyPatternDialog == null) {
				// if no instance of BibtexKeyPatternDialog exists, create new
				// one
				bibtexKeyPatternDialog = new BibtexKeyPatternDialog(
						JabRefFrame.this, basePanel());
			} else {
				// BibtexKeyPatternDialog allows for updating content based on
	...
	class IncreaseTableFontSizeAction extends MnemonicAwareAction {
		public IncreaseTableFontSizeAction() {
			...
			putValue(ACCELERATOR_KEY,
					Globals.prefs.getKey("Increase table font size"));
		}

		public void actionPerformed(ActionEvent event) {
			int currentSize = GUIGlobals.CURRENTFONT.getSize();
			Font newFont = new Font(GUIGlobals.CURRENTFONT.getFamily(),
					GUIGlobals.CURRENTFONT.getStyle(), currentSize + 1);
	...
	class DecreaseTableFontSizeAction extends MnemonicAwareAction {
		public DecreaseTableFontSizeAction() {
			...
			putValue(ACCELERATOR_KEY,
					Globals.prefs.getKey("Decrease table font size"));
		}
	...
	class MinimizeToSysTrayAction extends MnemonicAwareAction {
		public MinimizeToSysTrayAction() {
			...
			putValue(ACCELERATOR_KEY,
					Globals.prefs.getKey("Minimize to system tray"));
		}
	...
	public void showIfMinimizedToSysTray() {
		...
		// Workaround: put into invokeLater queue before a dialog is added to
		// that queue
		if (!this.isVisible()) {
